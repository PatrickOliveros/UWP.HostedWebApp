...
function toastActivationHandler(toastType, caller, rawdata) {
    ...
    switch (toastType) {
        ...
        case 'appToast':
            ...
            var redUrl = googleSearch + rawdata.requestNumber;
...
function showContactToast2(name, contactname, message, contactType) {
    if (isWindows10()) {
        var notifications = Windows.UI.Notifications;
        var template = notifications.ToastTemplateType.toastImageAndText02;
        var toastXml = notifications.ToastNotificationManager.getTemplateContent(template);
        ...
        var toastTextElements = toastXml.getElementsByTagName("text");
        toastTextElements[0].appendChild(toastXml.createTextNode("Message from " + name.capitalizeWord()));
...
function showCustomToastDropDown() {
    if (isWindows10()) {
        ...
        setToastActivation(toastDom, toastKey, "");
        setToastImage(toastDom, foodstore, 0,1);
        toggleToast(notifications, toastDom, toastKey);
...
function showContactToast(name, contactname, message, contactType) {
    if (isWindows10()) {
        var notifications = Windows.UI.Notifications;
        var template = notifications.ToastTemplateType.toastImageAndText02;
        var toastXml = notifications.ToastNotificationManager.getTemplateContent(template);
...
function getContactDetails(objContact) {
    ...
    var toastMessage = contactText.capitalizeWord() + " contact of " + objContact.displayName + " has been shared. " + contactInfo;
    ...
    showContactToast(getCurrentUser(), objContact.displayName, toastMessage, iconType);
