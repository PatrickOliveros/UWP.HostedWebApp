...
$(function() {
        ...
        chat.client.joinExistingConnections = function(connectionCount) {
            sendToClients(plainMessage(userNotification(-1,
                "",
                "You have " + connectionCount + checkPlural(connectionCount, " connection") + " in the room")));
        };
        ...
        chat.client.broadcastSelection = function(reqnumber) {
            if (isWindows10()) {
                showCustomToastDropDown(reqnumber);
            } else {
                // Todo: Do something, if needed
            }
        };
        ...
        chat.client.broadcastContactInformation = function(contactSharer,
            contactName,
            contactData,
            contactInfo,
            contactType) {
            if (isWindows10()) {
                showContactToast(contactSharer, contactName, contactData, contactInfo, contactType);
            } else {
                var message = contactSharer.capitalizeWord() +
                    " has shared contact information of [" +
                    contactName +
                    "] - [" +
                    contactInfo +
                    "]";
            ...
            $.connection.hub.start()
                .done(function() {
                        ...
                        $("#conTrigger")
                            .click(function() {
                                chat.server
                                    .broadcastContactToast($("#conSharer").val(),
                                        $("#conName").val(),
                                        $("#conData").val(),
                                        $("#conInfo").val(),
                                        $("#conType").val());
